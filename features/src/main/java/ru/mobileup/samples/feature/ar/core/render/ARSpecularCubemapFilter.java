 ...
 */
package ru.mobileup.samples.feature.ar.core.render;
 ...
 */
public class ARSpecularCubemapFilter implements Closeable {
  private static final String TAG = ARSpecularCubemapFilter.class.getSimpleName();
  ...
  private final ARTexture radianceCubemap;
  private final ARTexture ldCubemap;
  // Indexed by attachment chunk.
  private final ARShader[] shaders;
  private final ARMesh mesh;
  ...
  /**
   * Constructs a {@link ARSpecularCubemapFilter}.
   *
   ...
   */
  public ARSpecularCubemapFilter(ARSampleRender render, int resolution, int numberOfImportanceSamples)
      throws IOException {
    ...
    try {
      radianceCubemap =
          new ARTexture(render, ARTexture.Target.TEXTURE_CUBE_MAP, ARTexture.WrapMode.CLAMP_TO_EDGE);
      ldCubemap =
          new ARTexture(render, ARTexture.Target.TEXTURE_CUBE_MAP, ARTexture.WrapMode.CLAMP_TO_EDGE);
      ...
      ARVertexBuffer coordsBuffer = new ARVertexBuffer(render, COMPONENTS_PER_VERTEX, COORDS_BUFFER);
      mesh =
          new ARMesh(
              render,
              ARMesh.PrimitiveMode.TRIANGLE_STRIP,
              /*indexBuffer=*/ null,
              new ARVertexBuffer[] {coordsBuffer});
    } catch (Throwable t) {
      close();
      throw t;
    }
  ...
  public void update(Image[] images) {
    try {
      ...
      ARGLError.maybeThrowGLException("Failed to bind radiance cubemap texture", "glBindTexture");
      ...
      for (int i = 0; i < NUMBER_OF_CUBE_FACES; ++i) {
        ...
        ARGLError.maybeThrowGLException("Failed to populate cubemap face", "glTexImage2D");
      }

      GLES30.glGenerateMipmap(GLES30.GL_TEXTURE_CUBE_MAP);
      ARGLError.maybeThrowGLException("Failed to generate cubemap mipmaps", "glGenerateMipmap");
      ...
      for (int level = 0; level < numberOfMipmapLevels; ++level) {
        ...
        ARGLError.maybeThrowGLException("Failed to set viewport dimensions", "glViewport");
        for (int chunkIndex = 0; chunkIndex < shaders.length; ++chunkIndex) {
          ...
          ARGLError.maybeThrowGLException("Failed to bind cubemap framebuffer", "glBindFramebuffer");
   ...
   * #update(Image[])}.
   */
  public ARTexture getFilteredCubemapTexture() {
    return ldCubemap;
  }
  ...
  private void initializeLdCubemap() {
    ...
    ARGLError.maybeThrowGLException("Could not bind LD cubemap texture", "glBindTexture");
    for (int level = 0; level < numberOfMipmapLevels; ++level) {
      int mipmapResolution = resolution >> level;
      for (int face = 0; face < NUMBER_OF_CUBE_FACES; ++face) {
        GLES30.glTexImage2D(
            ...
            GLES30.GL_HALF_FLOAT,
            /*data=*/ null);
        ARGLError.maybeThrowGLException("Could not initialize LD cubemap mipmap", "glTexImage2D");
      }
    }
  }

  private ARShader[] createShaders(ARSampleRender render, ChunkIterable chunks) throws IOException {
    ...
    ARShader[] shaders = new ARShader[chunks.numberOfChunks];
  ...
  private int[][] createFramebuffers(ChunkIterable chunks) {
    ...
    for (int level = 0; level < numberOfMipmapLevels; ++level) {
      ...
      ARGLError.maybeThrowGLException("Could not create cubemap framebuffers", "glGenFramebuffers");
      for (Chunk chunk : chunks) {
        // Set the drawbuffers
        GLES30.glBindFramebuffer(GLES30.GL_FRAMEBUFFER, framebufferChunks[chunk.chunkIndex]);
        ARGLError.maybeThrowGLException("Could not bind framebuffer", "glBindFramebuffer");
        GLES30.glDrawBuffers(chunk.chunkSize, ATTACHMENT_ENUMS, 0);
        ARGLError.maybeThrowGLException("Could not bind draw buffers", "glDrawBuffers");
  ...
  private static int getMaxColorAttachments() {
    ...
    ARGLError.maybeThrowGLException("Failed to get max color attachments", "glGetIntegerv");
    return result[0];
  }
