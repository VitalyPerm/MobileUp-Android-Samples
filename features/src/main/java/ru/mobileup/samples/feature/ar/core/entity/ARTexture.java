...
/** A GPU-side texture. */
public class ARTexture implements Closeable {
  private static final String TAG = ARTexture.class.getSimpleName();
  ...
  /**
   * Construct an empty {@link ARTexture}.
   *
   * <p>Since {@link ARTexture}s created in this way are not populated with data, this method is
   ...
   */
  public ARTexture(ARSampleRender render, Target target, WrapMode wrapMode) {
    this(render, target, wrapMode, /*useMipmaps=*/ true);
  }

  public ARTexture(ARSampleRender render, Target target, WrapMode wrapMode, boolean useMipmaps) {
    this.target = target;

    GLES30.glGenTextures(1, textureId, 0);
    ARGLError.maybeThrowGLException("Texture creation failed", "glGenTextures");
  ...
  public static ARTexture createFromAsset(
          ARSampleRender render, String assetFileName, WrapMode wrapMode, ColorFormat colorFormat)
      throws IOException {
    ARTexture texture = new ARTexture(render, Target.TEXTURE_2D, wrapMode);
    Bitmap bitmap = null;
    try {
      ...
      GLES30.glTexImage2D(
          ...
          buffer);
      ARGLError.maybeThrowGLException("Failed to populate texture data", "glTexImage2D");
      GLES30.glGenerateMipmap(GLES30.GL_TEXTURE_2D);
      ARGLError.maybeThrowGLException("Failed to generate mipmaps", "glGenerateMipmap");
    } catch (Throwable t) {
      texture.close();
      throw t;
    } finally {
  ...
  public void close() {
    if (textureId[0] != 0) {
      ...
      ARGLError.maybeLogGLError(Log.WARN, TAG, "Failed to free texture", "glDeleteTextures");
      textureId[0] = 0;
    }
  }
