...
public class ARBackgroundRenderer {
  private static final String TAG = ARBackgroundRenderer.class.getSimpleName();
  ...
  private final ARVertexBuffer cameraTexCoordsVertexBuffer;
  private ARShader backgroundShader;
  private ARShader occlusionShader;
  private final ARTexture cameraDepthTexture;
  private final ARTexture cameraColorTexture;
  private ARTexture depthColorPaletteTexture;
   ...
   * during a {@link ARSampleRender.Renderer} callback, typically in {@link
   * ARSampleRender.Renderer onSurfaceCreated()}.
   */
  public ARBackgroundRenderer(ARSampleRender render) {
    cameraColorTexture =
        new ARTexture(
            render,
            ARTexture.Target.TEXTURE_EXTERNAL_OES,
    ...
    ARVertexBuffer screenCoordsVertexBuffer =
        new ARVertexBuffer(render, /* numberOfEntriesPerVertex=*/ 2, NDC_QUAD_COORDS_BUFFER);
    cameraTexCoordsVertexBuffer =
        new ARVertexBuffer(render, /*numberOfEntriesPerVertex=*/ 2, /*entries=*/ null);
    ARVertexBuffer virtualSceneTexCoordsVertexBuffer =
        new ARVertexBuffer(render, /* numberOfEntriesPerVertex=*/ 2, VIRTUAL_SCENE_TEX_COORDS_BUFFER);
    ARVertexBuffer[] vertexBuffers = {
      screenCoordsVertexBuffer, cameraTexCoordsVertexBuffer, virtualSceneTexCoordsVertexBuffer,
    };
    mesh =
        new ARMesh(render, ARMesh.PrimitiveMode.TRIANGLE_STRIP, /*indexBuffer=*/ null, vertexBuffers);
  }
   ...
   */
  public void setUseDepthVisualization(ARSampleRender render, boolean useDepthVisualization)
      throws IOException {
   ...
   */
  public void setUseOcclusion(ARSampleRender render, boolean useOcclusion) throws IOException {
    ...
    occlusionShader =
        ARShader.createFromAssets(render, "shaders/occlusion.vert", "shaders/occlusion.frag", defines)
            .setDepthTest(false)
            .setDepthWrite(false)
            .setBlend(ARShader.BlendFactor.SRC_ALPHA, ARShader.BlendFactor.ONE_MINUS_SRC_ALPHA);
    if (useOcclusion) {
      occlusionShader
          .setTexture("u_CameraDepthTexture", cameraDepthTexture)
   ...
   */
  public void drawBackground(ARSampleRender render) {
    render.draw(mesh, backgroundShader);
  }

  /**
   * Draws the virtual scene. Any objects rendered in the given ARFramebuffer will be drawn
  ...
  public void drawVirtualScene(
          ARSampleRender render, ARFramebuffer virtualSceneFramebuffer, float zNear, float zFar) {
