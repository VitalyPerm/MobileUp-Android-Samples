 ...
 *
 * <p>One or more {@link ARVertexBuffer}s are used when constructing a {@link ARMesh} to describe vertex
 ...
 */
public class ARVertexBuffer implements Closeable {
  private final ARGpuBuffer buffer;
  private final int numberOfEntriesPerVertex;

  /**
   * Construct a {@link ARVertexBuffer} populated with initial data.
   *
   ...
   * instead.
   */
  public ARVertexBuffer(ARSampleRender render, int numberOfEntriesPerVertex, FloatBuffer entries) {
    ...
    buffer = new ARGpuBuffer(GLES30.GL_ARRAY_BUFFER, ARGpuBuffer.FLOAT_SIZE, entries);
  }
