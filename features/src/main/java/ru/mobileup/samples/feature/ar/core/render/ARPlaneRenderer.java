...
import ru.mobileup.samples.feature.ar.core.entity.ARIndexBuffer;
...
public class ARPlaneRenderer {
  private static final String TAG = ARPlaneRenderer.class.getSimpleName();
  ...
  private final ARIndexBuffer indexBufferObject;
  private final ARVertexBuffer vertexBufferObject;
  private final ARShader shader;
   ...
   * a {@link ARSampleRender.Renderer} callback, typically in {@link
   * ARSampleRender.Renderer#onSurfaceCreated}.
   */
  public ARPlaneRenderer(ARSampleRender render) throws IOException {
    ARTexture texture =
        ARTexture.createFromAsset(
            render, TEXTURE_NAME, ARTexture.WrapMode.REPEAT, ARTexture.ColorFormat.LINEAR);
    shader =
        ARShader.createFromAssets(render, VERTEX_SHADER_NAME, FRAGMENT_SHADER_NAME, /*defines=*/ null)
            .setTexture("u_Texture", texture)
            .setVec4("u_GridControl", GRID_CONTROL)
            .setBlend(
                ARShader.BlendFactor.DST_ALPHA, // RGB (src)
                ARShader.BlendFactor.ONE, // RGB (dest)
                ARShader.BlendFactor.ZERO, // ALPHA (src)
                ARShader.BlendFactor.ONE_MINUS_SRC_ALPHA) // ALPHA (dest)
            .setDepthWrite(false);

    indexBufferObject = new ARIndexBuffer(render, /*entries=*/ null);
    vertexBufferObject = new ARVertexBuffer(render, COORDS_PER_VERTEX, /*entries=*/ null);
    ARVertexBuffer[] vertexBuffers = {vertexBufferObject};
    mesh = new ARMesh(render, ARMesh.PrimitiveMode.TRIANGLE_STRIP, indexBufferObject, vertexBuffers);
  }
  ...
  public void drawPlanes(
          ARSampleRender render, Collection<Plane> allPlanes, Pose cameraPose, float[] cameraProjection) {
