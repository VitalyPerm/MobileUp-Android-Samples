...
import ru.mobileup.samples.features.yandex_map.presentation.widget.MapOverlay
...
fun YandexMapUi(
    ...
    val placesPinsOverlay = remember { PlacePinsOverlay(component::onPlaceClick) }

    LaunchedEffect(placesState, placesPinsOverlay) {
        placesState.data?.let(placesPinsOverlay::updatePlaces)
    }

    val overlays = remember { mutableListOf<MapOverlay>(placesPinsOverlay, myLocationOverlay) }

//    LaunchedEffect(isCurrentLocationAvailable) {
//        if (isCurrentLocationAvailable) overlays.add(myLocationOverlay) else overlays.remove(myLocationOverlay)
//    }
